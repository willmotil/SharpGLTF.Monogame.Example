...
//    float lod = clamp(perceptualRoughness, 0.0f, 1.0f);
//    float3 reflection = normalize(reflect(-v, n));
//
//    float2 brdfSamplePoint = clamp(float2(NdotV, perceptualRoughness), float2(0.0f, 0.0f), float2(1.0f, 1.0f));
...
//{
//    float NdotV = clampedDot(n, v);
//    float lod = clamp(sheenRoughness * float(u_MipCount), 0.0f, float(u_MipCount));
//    float3 reflection = normalize(reflect(-v, n));
//
//    float2 brdfSamplePoint = clamp(float2(NdotV, sheenRoughness), float2(0.0f, 0.0f), float2(1.0f, 1.0f));
